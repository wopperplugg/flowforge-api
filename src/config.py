...
class Settings(BaseSettings):
    ...
    app_secret_key: SecretStr = Field(
        default=SecretStr("local-development-secret-change-me-32-bytes"),
    ...
    postgres_password: SecretStr = Field(
        default=SecretStr("flowforge"), alias="POSTGRES_PASSWORD"
    )
    ...
    outbox_active_poll_interval: float = Field(
        default=1.0,
        alias="OUTBOX_ACTIVE_POLL_INTERVAL",
        gt=0,
    )
    outbox_idle_poll_interval: float = Field(
        default=5.0,
